...
| **R8 robustness — depth** | ✅ **engine config, no parser** | 20 k-deep nesting → **clean `RangeError` (no segfault)** — QuickJS catches the stack overflow. A C-side depth *guard* was rejected: counting nesting correctly needs comment/string-aware brace matching = hand-rolled parsing (the forbidden thing; caused bug H4). Instead the depth bound is **`JS_SetMaxStackSize(rt, ~4 MB)`** (set *below* the OS stack) + catch the `RangeError` — equivalent to the core's `CSSON_MAX_DEPTH` reject-too-deep, via the engine, **no brace counting**. Measured: 256 KB→173, 1 MB→696, 4 MB→2788 frames, all clean RangeErrors. Optional trivial `len > N` input cap. |
