...
function coerce(raw){const s=String(raw).trim();
...
function nodeOf(rule){const o={};
  rule.nodes.forEach(n=>{ if(n.type==='decl'&&n.prop.startsWith('--'))o[n.prop.slice(2)]=coerce(n.value);
    else if(n.type==='rule'){const ty=seltype(n.selector);(o[ty]=o[ty]||[]).push(nodeOf(n));}});return o;}
...
function cssonCanon(text){const root=postcss.parse(text);let r=null;
...
function validate(syntax,value){ return globalThis.csstree.lexer.match(syntax,value).matched!==null; }
globalThis.csson={ canon: cssonCanon, validate };
