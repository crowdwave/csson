...
/* VALIDATE a value against a CSS `@property` syntax descriptor (e.g. "<integer>",
 * "<color> | auto", "*"). Returns 1 if the value matches, 0 if not, -1 on error
 * (with *err set). Matches the browser's @property value validation. */
 ...
 *
 * Open a document once into a `csson_sheet` (≈ new CSSStyleSheet + replaceSync),
 * then navigate an index-addressed tree of `csson_rule`s (≈ cssRules /
 * CSSStyleRule), read and write fields through the rule's style block (≈
 * getPropertyValue / setProperty / removeProperty), and insert/delete child
 * rules (≈ CSSGroupingRule.insertRule / deleteRule). Edits are comment-preserving
 * source splices and the sheet is updated in place; rule handles are owned by the
 * sheet (freed by csson_close) and stay valid across edits (indices may shift,
