...
chk "H3 inline field rm" '{"point":[{"y":2}]}' "$("$CS" rm "$tmp/a.csson" /point/0/x | "$CS" canon -)"

# H4 — a brace inside a comment in the removed node must not fool the matcher
printf 'cssonv1{ a { /* } */ --x: 1; } b { --y: 2; } }\n' > "$tmp/b.csson"
chk "H4 comment-brace node rm" '{"b":[{"y":2}]}' "$("$CS" rm "$tmp/b.csson" /a/0 | "$CS" canon -)"

# H2 — a value that breaks out of the declaration must be rejected (injection)
printf 'cssonv1{ --x: 1; }\n' > "$tmp/c.csson"
"$CS" set "$tmp/c.csson" /x '2;} evil{--z:9' >/dev/null 2>&1
chk "H2 injection rejected" "1" "$?"
chk "H2 legit set" '{"x":42}' "$("$CS" set "$tmp/c.csson" /x '42' | "$CS" canon -)"

# M3 — removing the document root must be rejected
"$CS" rm "$tmp/c.csson" / >/dev/null 2>&1
...
chk "single-line node insert" '{"dept":[{"name":"Eng"}],"x":1}' \
  "$("$CS" patch "$tmp/ins.csson" <(printf '[{"op":"add","path":"/dept/-","value":{"name":"Eng"}}]') | "$CS" canon -)"

# H1 — deeply nested input must error cleanly, not crash (no signal exit > 128)
python3 -c "open('$tmp/d.csson','w').write('cssonv1{'+'a{'*20000+'--x:1;'+'}'*20000+'}')"
"$CS" canon "$tmp/d.csson" >/dev/null 2>&1; rc=$?
