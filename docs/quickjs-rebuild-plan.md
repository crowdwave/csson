...
| **R4** | **Comment-preserving edits via `loc`** | 🟠 High | Edits must splice at exact offsets and keep comments/format. csstree `loc` must be byte-accurate (value start/end, block end). | **S4:** `set`/`remove` on `examples/sample_commented-csson.css` via csstree `loc` splice; diff that all comments + whitespace survive and the result re-parses. |
